- [Equilibrium Index](src/equilibrium_index.abap)
- [Extend Your Language](src/extend_your_language.abap)
- [Fibonnaci Sequence](src/fibonnaci.abap)
- [FizzBuzz](src/fizzbuzz.abap)
- [Generate lower case ASCII alphabet](src/generate_lower_case_ascii_alphabet.abap)
- [Guess the number](src/guess_the_number.abap)
- [Morse Code](src/morse_code.abap)
- [Show The Epoch](src/show_the_epoch.abap)
- [SoundEx (Incomplete)](src/soundex.abap) 
- [String Concatenation](src/string_concatenation.abap) 
